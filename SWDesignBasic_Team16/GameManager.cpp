...
void GameManager::pushStates(std::deque<Event*>& eventQueue)
{
	while (!eventQueue.empty()) {
		this->states.push(new EventState(this->window, eventQueue.front()));
...
void GameManager::update() {
	...
	if (!this->states.empty()) {
		if (!this->states.top()->eventQueue.empty()) {
			pushStates(this->states.top()->eventQueue);
